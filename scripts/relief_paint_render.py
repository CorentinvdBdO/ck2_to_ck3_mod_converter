...
REGIONS: dict[str, tuple[int, int, int, int]] = {
    ...
    # lane `relief-paint`: a Sword Coast HILL region (not the Waterdeep flat
    # coastal box `scripts/render_map_paint_evidence.py` uses) -- centred on
    # the same Sword Coast box docs/step_map_paint.md §9.4 already locates
    # (2080, 940, 2048, 2048 as x,y,w,h), 512 px around its own centre so
    # this region is the same size as thay/spine.
...
# --------------------------------------------------------------------------- #
# lane `relief-paint`: drape the paint tint over the relief
# --------------------------------------------------------------------------- #
#: illustrative colours per coarse relief category
#: (ck2ck3.map.relief_paint.CATEGORY_NAMES) -- NOT vanilla's measured
#: colormap tint (that is ck2ck3.map.colormap's job); this is only so a
#: render shows WHERE the paint's primary material category sits relative to
...
def load_paint_category_map(
    mod_dir: Path, categories_csv: Path, crop: tuple[slice, slice] | None = None
) -> np.ndarray:
    """(H, W, 3) uint8 -- the primary `detail_index` channel's category
    colour, at the mod's own paint resolution (may differ from the
    heightmap's if `terrain_paint_scale != 1.0`)."""
    import sys as _sys

    _sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "src"))
    from ck2ck3.map.relief_paint import (  # noqa: E402
        CATEGORY_NAMES, read_material_categories,
    )
    from ck2ck3.map.terrain_paint import load_paint, material_ordinals  # noqa: E402

    terrain_dir = mod_dir / "gfx" / "map" / "terrain"
    idx_path = next(
        (terrain_dir / f"detail_index.{ext}" for ext in ("tga", "dds")
         if (terrain_dir / f"detail_index.{ext}").exists()),
        None,
    )
    if idx_path is None:
        raise FileNotFoundError(f"no detail_index.(tga|dds) under {terrain_dir}")
    fmt = "dds" if idx_path.suffix == ".dds" else "tga"  # tga/tga_rle read the same
    idx = load_paint(idx_path, fmt)
    materials_settings = terrain_dir / "materials.settings"
    ordinals = material_ordinals(
        materials_settings if materials_settings.exists()
        else C.GAME / "gfx/map/terrain/materials.settings"
    )
    categories = read_material_categories(categories_csv)
    ...
    for o in range(256):
        ...
        cat_of_ordinal[o] = cat_code.get(categories.get(mat, "other"), cat_code["other"])
    primary_cat = cat_of_ordinal[idx[..., 0]]
    lut = np.array([_CATEGORY_RGB[c] for c in CATEGORY_NAMES], dtype=np.uint8)
    rgb = lut[primary_cat]
    if crop is not None:
        rgb = rgb[crop]
    return rgb
...
def save_paintshade(level: np.ndarray, category_rgb: np.ndarray, path: Path,
                     title: str, *, opacity: float = 0.45) -> None:
    """Hillshade with the paint's primary-material category tint draped over
    it at `opacity` -- the relief and the material choice in one image."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    z = world_height(level)
    shade = hillshade(z)
    gray = np.stack([shade] * 3, axis=-1)
    cat = category_rgb.astype(np.float64) / 255.0
    if cat.shape[:2] != gray.shape[:2]:
        # the paint pair may be at a different scale than the heightmap
        # (terrain_paint_scale); nearest-resample the category field onto
        # the heightmap's own grid rather than assume 1:1.
        hh, hw = gray.shape[:2]
        ch, cw = cat.shape[:2]
        rows = (np.arange(hh) * ch // hh).clip(0, ch - 1)
        cols = (np.arange(hw) * cw // hw).clip(0, cw - 1)
        cat = cat[rows[:, None], cols[None, :]]
    composite = gray * (1 - opacity) + cat * opacity
...
def main() -> None:
    ...
    ap.add_argument("--paint-mod", default=None,
                    help="lane `relief-paint`: a generated mod dir whose "
                         ...
                         "over each source's hillshade as a paint-colour "
                         "render (paintshade_<name>.png)")
    ap.add_argument(
        "--categories-csv",
    ...
    if args.paint_mod:
        ...
        print(f"loading paint from {args.paint_mod}...")
        paint_cat = load_paint_category_map(
            Path(args.paint_mod), Path(args.categories_csv), crop=(ys, xs)
        )
    ...
    for name, arr in sources.items():
        ...
        if paint_cat is not None:
            save_paintshade(
                arr, paint_cat, out / f"paintshade_{tag}{name}.png",
                f"{label} - {name} (paint-draped hillshade)",
            )
