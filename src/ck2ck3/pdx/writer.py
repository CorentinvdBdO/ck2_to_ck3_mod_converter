...
def write_file(
    ...
    **kwargs: object,
    ...
    """Write a parse tree to ``path`` as UTF-8 with LF endings.

    The BOM follows :data:`~ck2ck3.pdx.encoding.OUT_ENCODING`: present iff the
    rendered text is not pure ASCII, which is what vanilla CK3 does and what
    ck3-tiger expects. Pass ``encoding=OUT_PLAIN_ENCODING`` to force none.
