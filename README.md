...
| `traits` | `common/traits/*` (1417 traits) | `common/traits`, trait icons | 400 ported (117 race traits get `genetic`, `physical`, `life_expectancy` from `overrides/race_lifespan.csv`), 142 that CK3 already has exactly or nearly enough are **mapped to that existing CK3 trait**, never redefined (rename map `mappings/trait_id_map.csv`), 867 one-off biography traits emitted commented. A CK2 vanilla trait CK3 removed with no CK3 equivalent at all either becomes a `sexuality` history key (`homosexual`, 1) or is dropped with a `# CK2 trait x: no CK3 counterpart` comment (7) — never ported as a new CK2-named trait, so the player never sees `cruel`/`envious`/etc. in the trait list. A dedupe that gives a character two conflicting CK3 traits (duplicate, CK3 `opposites`, or two levels of one leveled trait) keeps the CK2-first-listed one and comments the rest. Modifiers through `mappings/modifiers.csv` with scale factors derived from same-trait pairs. `docs/step_traits.md`, `docs/mapping_modifiers.md`. |
