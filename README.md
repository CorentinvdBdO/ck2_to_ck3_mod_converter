...
| `traits` | `common/traits/*` (1417 traits) | `common/traits`, trait icons | 407 ported (117 race traits get `genetic`, `physical`, `life_expectancy` from `overrides/race_lifespan.csv`), 143 that CK3 already has are **not redefined** (rename map `mappings/trait_id_map.csv`), 867 one-off biography traits emitted commented. Modifiers through `mappings/modifiers.csv` with scale factors derived from same-trait pairs. `docs/step_traits.md`, `docs/mapping_modifiers.md`. |
